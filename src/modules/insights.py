...
class InsightsModule(BaseModule):
    ...
    def get_insight_details(
            ...
            assets affected by this insight, use the `get_assets_by_insight_name` tool.
            """
            try:
                insight_path = urllib.parse.quote(insight_name.strip())
                ...
                while True:
                    ...
                    if current_page == 1:
                        ...
                        # Text Cleaning
                        clean_desc = re.sub(r'\[\[(.*?)\$\$.*?\]\]', r'\1', raw_desc)
                ...
                # Table headers (no longer appending filter_key)
